...
public class Main {
    public static void main(String[] args) {
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
            while (true) {
                String res = calc(reader.readLine());
    ...
    private static void checkInputNumber(int i) throws Exception {
        if (i < 1 || i > 10)
            throw new Exception("Введенное число \"" + i + "\" не соответствует заданию - число не входит в предел [1, 10]");
    }

    private static void checkResultForRomanNumbers(int i) throws Exception {
        if (i < 1)
